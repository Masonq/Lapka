 ...
 *
 * Своя всплывающая панель форматирования (Bold/Italic/Underline/
 * Strikethrough/Link/Spoiler) была реализована и убрана по решению
 * пользователя — iOS уже показывает нативное меню "Форматирование"
 * (Bold/Italic/Underline) при выделении текста в contenteditable, свой
 * дублирующий тулбар поверх этого создавал только визуальный шум.
 * Конвертация нативного форматирования (<b>/<i>/<u>, вставляемых через
 * системное меню) в markdown-синтаксис ниже остаётся — она относится к
 * ЧТЕНИЮ текста, не к панели, и продолжает работать без неё.
 */
export default function ScrollableTextarea({
  ...
  useEffect(() => {
    ...
    // маркеры — рекурсивно обходим дерево узлов, конвертируя это в
    // тот же markdown-синтаксис, что использует вся остальная система
