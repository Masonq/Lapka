...
| 18 | Services / Business | 🟡 | Каталог услуг + отзывы + рейтинг + verified badge (проставляет только админ, отдельный раздел в админке) работают. Нет: часы работы, карта, booking (сам блюпринт помечает booking как Phase 3) |
...
| 22 | Profile | 🟡 | Аватар/имя/город/питомцы/сохранённое/настройки/выход есть. Кликабельные счётчики подписчиков/подписок на своей и чужой странице — ведут на список /users/:id/connections. Blocked users работает: блокировка двусторонняя (нельзя написать/подписаться в обе стороны), рвёт существующие подписки, список и разблокировка в настройках. Нет: privacy, security (кроме смены пароля) |
| 23 | Settings | 🟡 | Смена пароля, удаление аккаунта (с каскадным удалением всех данных — питомцы/посты/комментарии/подписки/отзывы/уведомления), список заблокированных с разблокировкой. Нет: notifications settings, location, language, appearance, data export, help & support |
| 24–25 | Admin Dashboard + модерация | 🟡 | Overview (счётчики), очередь жалоб на посты И объявления (dismiss/удалить), audit log, верификация исполнителей услуг (verified badge) — реально работают, is_admin проверяется на бэкенде и не светится в публичной схеме пользователя. Нет: Users/Pets/Communities/Businesses вкладок, feature flags, apologize-flow при апелляции |
...
| 31 | MVP Release Gates | 🟡 | Есть: тесты (214), rate limiting, миграции, non-root Docker, CORS, audit log для действий модерации, все списочные эндпоинты проверены на N+1, реальный деплой на VPS, бэкапы (deploy/backup.sh — БД+файлы, ротация 14 дней, cron-инструкция) + проверенный deploy/restore.sh. Нет: E2E, permissions-аудит по ролям, Terms/Privacy/Guidelines, off-site копия бэкапов (только локально на том же сервере — честно отмечено в самом скрипте) |
